...
/**
 * Provides the core implementation for the CBOR indexed map data item.
 *
 * @author Chris Holgate
 */
final class IndexedMapDataItem extends DataItemCore<Map<Long, DataItem<?>>> {

  // This is the map of data items which make up the CBOR indexed map.
  private final Map<Long, DataItem<?>> mapData;

  /**
   * Provides a constructor for use by the data item factory API. This creates an
   * empty indexed map data item which can then be populated by application code.
   *
   ...
   *   map.
   */
  IndexedMapDataItem(final int[] tags, final boolean indefiniteLength) {
    super(UserDataType.INDEXED_MAP, tags, true, indefiniteLength);
    mapData = new HashMap<Long, DataItem<?>>();
  }
   ...
   * application code.
   *
   * @param mapData This is the map of integer keys to CBOR data items which are
   *   to be included in the indexed map.
   * @param tags This is an array of integer values which represent the tags to be
   *   applied to the data item.
   ...
   *   formatting.
   */
  IndexedMapDataItem(final Map<Long, DataItem<?>> mapData, final int[] tags, final boolean indefiniteLength) {
    super(UserDataType.INDEXED_MAP, tags, false, indefiniteLength);
   ...
   */
  @Override
  public Map<Long, DataItem<?>> getData() {
    return mapData;
  }
  ...
  void appendCbor(final DataOutputStream outputStream) throws IOException {
    ...
    // Write out the indexed map using the CBOR indefinite length format.
    if (isIndefiniteLength()) {
      writeIndefinitePrimaryData(MajorType.MAP, outputStream, getTags());
      for (final Map.Entry<Long, DataItem<?>> mapEntry : mapData.entrySet()) {
        if (mapEntry.getKey() >= 0) {
          writePrimaryData(MajorType.UNSIGNED, mapEntry.getKey(), outputStream, getTags());
        } else {
          writePrimaryData(MajorType.NEGATIVE, ~mapEntry.getKey(), outputStream, getTags());
        }
    ...
    // Write out the indexed map using the CBOR fixed length format.
    else {
